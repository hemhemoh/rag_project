...
class ArxivModel():
    ...
    def query_processor(self, query: str) -> list[str]:
        prompt = PromptTemplate(
            input_variables=["query"],
            template=(
                "You are a research assistant who helps people stay up to date on the latest research trends. "
                "The user has provided the following query: '{query}'.\n"
                "Please extract and return a comma-separated list of relevant keywords only. "
                "No extra text, no explanation—just the keywords."
            )
        )
        keyword_chain = LLMChain(llm=self.chat_model, prompt=prompt)
        keywords_str = keyword_chain.run({"query": query})
        keywords = [k.strip() for k in keywords_str.split(",") if k.strip()]
        # print("Keyword output:", keywords)
        return keywords
    ...
    def get_arxiv_model(self, query):
